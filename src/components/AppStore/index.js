...
class AppStore extends Component {
  ...
  renderAppList = () => {
    ...
    const appItemsDetails = appsList.map(
      eachApp => eachApp.category === activeTabId,
    )
    const searchResults = appItemsDetails.filter(eachItem =>
  ...
  render() {
    ...
    const searchResults = this.renderAppList()
    ...
    return (
      <div className="app-con">
        <div className="responsive-con">
          ...
          <ul className="appList-con">
            {searchResults.map(eachResult => (
              <AppItem key={eachResult.appId} appDetails={eachResult} />
            ))}
          </ul>
        </div>
      </div>
